...
def better(cand, best):
    ...
    if cand is None:
        return False
    return (best is None or cand[0] > best[0]
...
def wandb_history(run_id, keys):
    return list(api().run(f"{api().default_entity}/{WANDB_PROJECT}/{run_id}")
...
def check_pomo(tag, lineage, target):
    ...
    try:
        # the two metrics are logged in separate wandb.log calls (one row each,
        # no epoch key); the i-th row of each belongs to the i-th validation,
        # which is epoch i+1 up to resume gaps (POMO validates every epoch)
        feas_rows = wandb_history(run_id, [feas_key, "_timestamp"])
    ...
    points = []  # (feas, score, timestamp, index)
    for i, row in enumerate(feas_rows):
        score = as_score(score_rows[i][score_key]) if i < len(score_rows) else float("inf")
        points.append((100. - float(row[feas_key]), score, row["_timestamp"], i))
    gbest = None
    for p in points:
    ...
    meta = read_meta(d)
    if meta is not None and not better(gbest[:2], (meta["feas"], as_score(meta["score"]))):
    ...
    boundary = time.mktime(
        time.strptime(attaches[-1].split("-")[1], "%Y%m%d_%H%M%S")) - 60
    post = None
    for p in points:
        if p[2] >= boundary and better(p[:2], post and post[:2]):
            post = p
    if not better(gbest[:2], post and post[:2]):
        return "OK", f"{desc}; re-attained after the last resume ({len(attaches)} attaches)"
    lost = (f"only feas {post[0]:.2f}% score {post[1]:.4f} since the last resume"
            if post else "no validation since the last resume yet")
...
def check_am(tag, lineage, target):
    ...
    for r in rows:
        cand = (100. - float(r["val/infsb_rate"]), as_score(r["val/feasible_cost"]),
                int(r["epoch"]))
    ...
    holder = next((d for d in lineage
                   if epoch_ckpts(d) and epoch_ckpts(d)[-1][0] >= gbest[2]
                   and (d / "val_best.pt").is_file()), None)
    if holder is None:
...
def main():
    ...
    ap.add_argument("--sizes", type=int, nargs="+", default=[20, 50, 100],
                    choices=[20, 50, 100])
    ap.add_argument("--tws", type=str, nargs="+", default=None,
                    choices=["sw", "mw"], help="filter n100 window types")
    ap.add_argument("--variants", type=str, nargs="+", default=list(VARIANTS),
    ...
    if counts["AT RISK"]:
        print("AT RISK runs: the overwritten best may still be re-attained if the "
